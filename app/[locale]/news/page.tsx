...
export default function NewsPage() {
  ...
  // Quản lý riêng trailer (lấy không phân trang hoặc lấy mới nhất)
  ...
  // Lấy trailers (không phụ thuộc vào filter)
  const fetchTrailers = useCallback(async () => {
    try {
      const res = await fetch("/api/news")
      const data = await res.json()
      if (res.ok) {
        // Lọc ra các tin có videoUrl làm trailer, lấy 4 bài
        const allNews = Array.isArray(data) ? data : (data.news || [])
        setTrailers(allNews.filter((item: any) => item.videoUrl).slice(0, 4))
      }
    } catch (error) {
  ...
  // Loại bỏ các video trailers khỏi danh sách hiển thị chính (nếu muốn)
  // Hoặc cứ giữ nguyên vì API đã trả về đúng nội dung tìm kiếm
  const articles = news.filter(item => !item.videoUrl || category !== "All")
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex items-center gap-4 text-xs text-muted-foreground font-medium uppercase tracking-wider">
                                            ...
                                            <div className="flex items-center gap-1">
                                                ...
                                                {item.views?.toLocaleString() || 0} lượt xem
                                            </div>
